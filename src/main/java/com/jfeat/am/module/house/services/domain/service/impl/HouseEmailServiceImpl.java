...
//做处理
     ...
     */
    @Override
    public void sendEmailByCustomization(String emailTitle, String emailContent, List<String> toEmailAddressList) {

        // 如果toEmailAddressList为空, 则使用系统默认收件人邮件列表
        if (toEmailAddressList.isEmpty()) {
            toEmailAddressList = FileUtility.readFileToList(EMAIL_LIST_PATH);
        }
        final List<String> toEmailAddresses = toEmailAddressList;

        // 组织email内容
        WorkTaskModel workTaskModel = new WorkTaskModel();
        workTaskModel.setTaskName(emailTitle); // 邮件标题
        workTaskModel.setDesc(emailContent); // 邮件内容
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                for (String toEmailAddress : toEmailAddresses) {
                    // 单独使用toEmailAddress不能成功发送邮件，原因未知，所以使用temporaryEmailAddressList
                    /*
                    为什么要再创建一个temporaryEmailAddressList？
                    是因为在收件人的邮件里可以看到全部的temporaryEmailAddressList，
                    这样会造成用户邮箱泄露，所以创建多一个临时的列表，每个列表里面只放一个收件人邮箱
                     */
                    List<String> temporaryEmailAddressList = new ArrayList<>();
                    temporaryEmailAddressList.add(toEmailAddress);
                    workTaskModel.setToEmailAddressList(temporaryEmailAddressList);
                    // 发送email
                    taskMessageService.sendTaskMessageToKafkaEmail(workTaskModel);
                }
            }
        }).start();
    }
    ...
    private WorkTaskModel createWorkTask(String queueName, String taskName, String msg, Long
            userId, List<String> emailList) {
        ...
        // task表中priority不允许为空，优先级 0 1 2 从低到高，目前并不知道优先级的作用，此处先行使用2
